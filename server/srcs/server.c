...
int	check_all_fd(struct timeval *tv, server_t *server,
	double backup_time, fd_set readfds)
{
	if (tv)
		free(tv);
...
int	check_fd(t_parse *parse, server_t *server, fd_set readfds)
{
	...
	ret = select((server->fd > best_fd ? server->fd : best_fd) + 1,
